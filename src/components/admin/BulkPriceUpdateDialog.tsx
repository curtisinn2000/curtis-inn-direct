...
import { format, addDays, differenceInCalendarDays, getDay, startOfToday } from 'date-fns';
...
export function BulkPriceUpdateDialog({ open, onOpenChange }: Props) {
  ...
  const [updates, setUpdates] = useState<Record<string, RoomUpdate>>(
    () => Object.fromEntries(rooms.map(r => [r.id, { mode: 'no_change' as Mode, value: '' }]))
  );

  const reset = () => {
    setStep('dates'); setRange(undefined); setDays([0,1,2,3,4,5,6]);
    setUpdates(Object.fromEntries(rooms.map(r => [r.id, { mode: 'no_change' as Mode, value: '' }])));
  };
  ...
  return (
    <Dialog open={open} onOpenChange={close}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-xl">Bulk Update Prices</DialogTitle>
          ...
          <div className="space-y-4">
            <Card className="p-4 space-y-4">
              ...
              <div>
                ...
                <Calendar mode="range" selected={range} onSelect={setRange} numberOfMonths={2} disabled={(d) => d < today} className="pointer-events-auto" />
